...
Cada modelo tem seu próprio `tokenTimestamps[]` em janela deslizante configurável (padrão 5 min via `tpmWindowMs`). `MAX_TPM` (padrão 250K) é uma taxa por minuto — o orçamento real escala com a janela: `maxTpm * (tokenWindowMs / 60000)`. Antes do disparo, o custo estimado (tokens de prompt do `js-tiktoken` + `COMPLETION_BUFFER`) **mais tokens pendentes em voo** é verificado contra o orçamento disponível. Tokens pendentes são subtraídos na conclusão (mínimo 0). Rotas não-inference (`/v1/models`, etc.) pulam a verificação TPM. Composta como porta AND com RPM — ambas devem passar. Override por modelo: `tokenWindowMs`.
